...
def export_to_cad(geo_data, output_path, max_lat, min_lon, zoom, active_layers=None):
    ...
    for feature in features:
        ...
        if len(pixel_points) > 1:
            polyline = msp.add_lwpolyline(
                pixel_points, dxfattribs={"layer": layer_name}
            )
            ...
            is_structural = any(
                k in layer_name for k in ["Building", "Terminal", "Hangar"]
            )
